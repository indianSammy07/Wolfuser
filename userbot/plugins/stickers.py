...
async def _(event):
    ...
    #packname = f"WOLFUSERBOT PACK"
    #packshortname = f"WOLFUSERBOT_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 1244074215:
        packname = f"@RkProjects Packs 🎭"
        packshortname = "WOLFUSERBOTPack"
    else:
        packname = f"{user.first_name}'s WOLFUSERBOT Vol.{pack}"
        packshortname = f"WOLFUSERBOT_{userid}_Pack"
    ...
    if is_a_s:
        ...
        if userid == 1244074215:
            ...
            packname = f"{user.first_name}'s WOLFUSERBOT Animated Vol.{pack}"
            packshortname = f"WOLFUSERBOT_{userid}" # format: Uni_Borg_userid
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    packshortname = f"Vol_{pack}_with_{user.first_name}"
                       ...
                       # packshortname = "Vol._{pack}_WOLFUSERBOT_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_WOLFUSERBOT{userid}"
                        #packshortname = f"Vol._{pack}_WolfUserBot_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
                ...
                await silently_send_message(bot_conv, sticker_emoji)
                await silently_send_message(bot_conv, "/done")
    await event.edit(f"**Sticker** `Kanged Legally And It Can Be Found` [♻️Here♻️](t.me/addstickers/{packshortname})")
